  ...
  return (
    <div className="space-y-8 animate-fade-in-up px-2 pb-10">
      ...
      <div className="glass p-2 rounded-2xl border-none w-fit flex items-center gap-2">
        <div className="flex items-center gap-3 px-4 py-2 bg-foreground/5 rounded-xl border border-foreground/10">
          <CalendarIcon className="h-4 w-4 text-cyan-400" />
          <span className="text-[10px] font-bold uppercase tracking-[0.2rem] text-foreground/40">Active Schedule</span>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {/* Desktop Header */}
        <div className="hidden lg:grid grid-cols-[2fr_1.5fr_1.5fr_1.5fr_1.2fr_auto] gap-4 px-8 py-2 text-[10px] font-bold uppercase tracking-[0.3em] text-foreground/30">
              ...
              >
                <div className="grid grid-cols-1 lg:grid-cols-[2fr_1.5fr_1.5fr_1.5fr_1.2fr_auto] items-center gap-4 lg:gap-6">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] text-foreground/30 uppercase tracking-widest font-semibold">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-semibold text-foreground/80">{event.picName}</span>
                  </div>

                {/* Dates */}
                <div className="space-y-1">
                  <p className="text-[10px] text-foreground/20 uppercase tracking-widest font-bold">Start</p>
                  <span className="text-xs font-bold text-foreground/70">{formatDate(event.date)}</span>
                </div>

                <div className="space-y-1">
                  <p className="text-[10px] text-foreground/20 uppercase tracking-widest font-bold">End</p>
                  <span className="text-xs font-bold text-foreground/70">{formatDate(event.endDate)}</span>
                </div>
          ...
          <Select value={pic} onValueChange={setPic}>
            ...
            <SelectContent className="glass-darker border-foreground/10 text-foreground backdrop-blur-3xl">
              {users
                ...
                .map((u) => (
                  <SelectItem key={u.id} value={String(u.id)} className="focus:bg-cyan-500/20 focus:text-foreground cursor-pointer">
                    {u.name} ({u.role})
                  </SelectItem>
                ))}
            </SelectContent>
