...
export function CreatePersonalJobModal({ open, onOpenChange, onSuccess }: CreatePersonalJobModalProps) {
    ...
    return (
        <ModalForm open={open} onOpenChange={onOpenChange} title="Create Personal Job">
            <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-2">
                    <Label htmlFor="pj-name">Job Name</Label>
                    <Input id="pj-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Daily Machine Check" />
                </div>
                <div className="space-y-2">
                    <Label htmlFor="pj-desc">Description</Label>
                    <Textarea id="pj-desc" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Describe the job..." rows={2} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-2">
                        <Label htmlFor="pj-due">Due Date</Label>
                        <Input id="pj-due" type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} />
                    </div>
                    <div className="space-y-2">
                        <Label>Priority</Label>
                        <Select value={priority} onValueChange={(v) => setPriority(v as PersonalJobPriority)}>
                            <SelectTrigger>
                                <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                        ...
                        {checklistItems.map((item, index) => (
                            <div key={index} className="flex items-center gap-2">
                                ...
                                {checklistItems.length > 1 && (
                                    <Button type="button" variant="ghost" size="icon" onClick={() => removeChecklistItem(index)} className="h-9 w-9 shrink-0 text-destructive hover:text-destructive">
                ...
                <Button type="submit" className="w-full min-h-[44px]">Create Job</Button>
            </form>
        </ModalForm>
    );
}
