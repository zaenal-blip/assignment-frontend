...
export function TaskCard({ task }: TaskCardProps) {
    ...
    return (
        <Card
            className="group glass hover:bg-foreground/[0.08] cursor-pointer animate-fade-in transition-all duration-300 hover:scale-[1.02] hover:cyan-glow border-none rounded-3xl"
            onClick={() => navigate(`/tasks/${task.id}`)}
        >
            <CardContent className="p-5 tv:p-8 space-y-4">
                <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1 space-y-1">
                        <h3 className="font-bold text-foreground group-hover:text-cyan-400 transition-colors truncate tv:text-tv-base font-display tracking-tight">
                            {task.name}
                        </h3>
                    ...
                    <div className="flex flex-col items-end gap-2 shrink-0">
                        {pic && <AvatarBadge user={pic} size="sm" className="ring-2 ring-foreground/10 group-hover:ring-cyan-500/50 transition-all" />}
                        <StatusBadge 
                            status={task.status} 
                            className={cn(
                ...
                <div className="space-y-2">
                    <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-wider">
                        <span className="text-foreground/20">Progress</span>
                        <span className="text-cyan-400 text-glow">{progress}%</span>
                    </div>
                    <div className="h-1.5 w-full bg-foreground/5 rounded-full overflow-hidden p-[0.5px]">
                        <div 
